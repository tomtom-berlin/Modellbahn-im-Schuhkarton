...
#
# Funktionen für den Bereich "ELECTRICAL" der NMRA DCC RP 9.2 und "OPERATIONS" RP 9.2.1
#
# ----------------------------------------------------------------------
#
...
#
#         110: Advanced Operation Instructions 110GGGGG
#                    {instruction bytes} = 110GGGGG 0 DDDDDDDD
#                    GGGGG = 11110 = F13-F20 Function Control
#                            DDDDDDDD = State of function (0 = off, 1 = on)
#                            DDDDDDDD = F20 F19 F18 F17 F16 F15 F14 F13
#
#                    GGGGG = 11111: F21-F28 Function Control
#                            DDDDDDDD = F28 F27 F26 F25 F24 F23 F22 F21
#
#                    GGGGG = 11000: F29-F36 Function Control
#                            DDDDDDDD = F36 F35 F34 F33 F32 F31 F30 F29
#
#                    GGGGG = 11001: F37-F44 Function Control
#                    GGGGG = 11010: F45-F52 Function Control
#                    GGGGG = 11011: F53-F60 Function Control
#                    GGGGG = 11100: F61-F68 Function Control
#                            DDDDDDDD = F{max}...F{min}
#
#
...
class ELECTRICAL:
    ...
    LMD18200_QUIESCENT_CURRENT = 17.0
    DRV8871_QUIESCENT_CURRENT = 13.0
    LMD18200_SENS_SHUNT = 20000                # Ohm
    AREF_VOLT = 3300                           # mV !!
    DENOISE_SAMPLES = 200                      # Anzahl der Messzyklen, für Rauschunterdrückung
    LMD18200_SENS_AMPERE_PER_AMPERE = 0.000377 # Empfindlichkeit: 377µA / A lt. Datenblatt
    SHORT = 1000                               # erlaubter max. Strom in mA
    PREAMBLE = 14                              # Präambel f. Servicemode
    ACK_TRESHOLD = 40                          # Hub f. Ack
    CURRENT_SMOOTHING = 0.175                  # Glättung der Messergebnisse versuchen
    ...
    cls.__init__(cls, H_BRIDGE, DIR_PIN, BRAKE_PIN, PWM_PIN, POWER_PIN, ACK_PIN):
...
class OPERATIONS(ELECTRICAL):
    ...
    LEFT_OR_STOP = 0      # Parameter 'R': Fahrweg nach links bzw. Signal rot lt. RP 9.2.1 Abschnitt 2.4.1 
    RIGHT_OR_TRAVEL = 1   # Parameter 'R': Fahrweg nach rechts bzw. Signal grün
    DEACTIVATE = 0        # Parameter 'D': Aktivieren oder deaktivieren des angesprochenen Zubehörs
    ACTIVATE = 1          # Parameter 'D': Aktivieren oder deaktivieren des angesprochenen Zubehörs
    
    active_loco = None # Active Loco
    device = None # Active accessory
    
    #
    def __init__(cls, H_BRIDGE, DIR_PIN, BRAKE_PIN, PWM_PIN, POWER_PIN, ACK_PIN):
    ...
    # private Funktion
    # AUX-bits setzen und an Lok senden
    def __get_aux(cls, aux):

    # Toggle function
    def toggle_function(cls, function_nr)
...
class ELECTRICAL:
    ...
    aux_instructions = [0b11011110, 0b11011111, 0b11011000, 0b11011001, 0b11010010, 0b11011011, 0b11011100]
    # DCC- und H-Bridge-LMD18200T-Modul elektrische Steuerung
    #
#    @classmethod
    def __init__(cls, H_BRIDGE, DIR_PIN, BRAKE_PIN, PWM_PIN, POWER_PIN, ACK_PIN):
        cls.motordriver = H_BRIDGE  
        cls.brake = machine.Pin(BRAKE_PIN, machine.Pin.OUT)
        cls.pwm = machine.Pin(PWM_PIN, machine.Pin.OUT)
        cls.power = machine.Pin(POWER_PIN, machine.Pin.OUT)
        cls.dir_pin = machine.Pin(DIR_PIN, machine.Pin.OUT)
        ...
        cls.buffer_dirty = False
        cls.emergency = False
        cls.set_initial_state()

    @classmethod
    def set_initial_state(cls):
        cls.ringbuffer = []
        cls.accessory_buffer = [] # Accessory-Commands
        cls.pom_buffer = [] # POM-Commands
        cls.locos = [] # active Locos
        ...
        cls.statemachine = bitgenerator(cls.dir_pin, model=cls.motordriver)
#        cls.statemachine.begin()
    ...
    @classmethod
    def reset(cls):
        cls.__init__(cls.motordriver, cls.brake, cls.pwm, cls.power, cls.dir_pin, cls.ack)        
        for i in range(20):
            cls.ringbuffer.append(cls.RESET[0])
            cls.ringbuffer.append(cls.RESET[1])
        for i in range(10):
    ...
    def generate_instructions(cls):
        ...
        for loco in cls.locos:
            ...
            for aux_nr in range(len(loco.aux)):
                instruction = []
                if loco.use_long_address:
                    instruction.append(192 | (loco.address // 256))
                    instruction.append(loco.address & 0xff)
                ...
                instruction.append(cls.aux_instructions[aux_nr])
                instruction.append(loco.aux[aux_nr])
...
class OPERATIONS(ELECTRICAL):
    ...
    @classmethod
    def __init__(cls, H_BRIDGE, DIR_PIN, BRAKE_PIN, PWM_PIN, POWER_PIN, ACK_PIN):
        super().__init__(H_BRIDGE, DIR_PIN, BRAKE_PIN, PWM_PIN, POWER_PIN, ACK_PIN)
        
    @classmethod
    def ctrl_loco(cls, address=3, use_long_address=False, speedsteps=28, name=""):
        index = cls.search(address)
    ...
    @classmethod
    def search(cls, search_address):
        for i in range(len(cls.locos)):
            if cls.locos[i].address == search_address:
                return i
        return None
    ...
    # AUX-bits lesen und an Lok senden
    #
    @classmethod
    def __get_aux(cls, aux):
        grp = (aux - 13) // 8
        aux_msk = aux - ((grp * 8) + 13)
        return (cls.active_loco.aux[grp] & (1 << aux_msk)) != 0 
    ...
    def get_function(cls, function_nr):
        ...
        elif 13 <= function_nr <= 68:
            status = cls.__get_aux(function_nr)
        return status
    ...
    @classmethod
    def __set_aux(cls, aux, status):
        ggggg = [0b11011110, 0b11011111, 0b11011000, 0b11011001, 0b11011010, 0b11011100] 
        grp = (aux - 13) // 8
        aux_msk = aux - ((grp * 8) + 13)
        cccggggg = ggggg[grp]
        if(status == True):
            cls.active_loco.aux[grp] |= 1 << aux_msk
        else: 
            cls.active_loco.aux[grp] &= ~(1 << aux_msk)
    ...
    def set_function(cls, function_nr, status = True):
        if 0 <= function_nr <= 12:
            ...
            cls.__set_aux(function_nr, status)
        cls.buffer_dirty = True
        
    # Funktionsbits umschalten und an Lok senden
    #
    @classmethod
    def toggle_function(cls, function_nr):
        status = cls.get_function(function_nr)
        cls.set_function(function_nr, 0 if status else 1)
        cls.buffer_dirty = True
    ...
    #
    @classmethod
    def ctrl_accessory_extended(cls, address=1, aspect=0):
        b1 = 0b10000000
        b2 = 0b01110001
        b3 = 0b00000000

        device = ACCESSORY(address, signal=True, aspect=aspect)
        i = cls.search_accessory(device.address)
        if(i != None):
            cls.accessories[i].aspect = aspect
        else:
            cls.accessories.append(device)
...
class ACCESSORY:
    # new accessory
    def __init__(self, address=1, R=0, D=0, aspect=0, signal=False, timed = False, name=""):
...
class LOCO:
    ...
    def __init__(self, address=None, use_long_address=False, speedsteps=28, name=""):
        if address != None:
            ...
            # AUX = Functions 13-68
            self.aux = [0b00000000, 0b00000000, 0b00000000, 0b00000000, 0b00000000, 0b00000000, 0b00000000]
            if(name != None):
                self.name = name
