...
import time
from machine import Timer, Pin, UART, idle, reset, PWM, deepsleep, I2C
import os, re
...
if mcu == "XIAO-RP2040":
    ...
    USER_LED_PINS = [const(25), const(16), const(17)]
    ...
    # MCU RP pico / pico 2 / Waveshare RP2040 + 1.28 round LCD
    DIR_PIN = 19
    BRAKE_PIN = 20
    PWM_PIN = 21
    POWER_PIN = 22
    ...
    SDA = 4
    SCL = 5
    EMERG_PIN = 13  # Eingang: dieser Pin auf LOW und POWER_PIN schaltet ab
    RESET_PIN = None
...
def uart_begin(ch, TX, RX):
    uart = UART(ch, baudrate=115200, tx=Pin(TX), rx=Pin(RX))
    return uart
...
def show_current(t):
    i = op.get_current()
    duty_high_current = m.map(i, 0, 1000, 0, 4096)
    duty_low_current = 4096 - duty_high_current
    if(mcu == "XIAO-RP2040"):
        ...
        if op.active_loco != None:
            if __DEBUG__ == 2:
                print(f"'{op.active_loco.name}' [{op.active_loco.address}]")
            addr = op.active_loco.address
            direction = op.direction()
            speed = op.speed()
            response(5, 5, f"Lok:{addr:^5} FS:{'->>' if direction == 1 else '<<-':^3}{speed:^3}")
            s = ""
            if op.active_loco != None:
                for f in range(0, 13):
    ...
    if(__DEBUG__ > 1):
        log_print(f"DCC-Strom: {i:>4} mA, duty_high={duty_high_current}, duty_low={duty_low_current}", end="\r")
...
if (USE_UART):
    # UART-Eingabe
    uart = uart_begin(UART_CHANNEL, UART_TX, UART_RX)
else:
    # Tastatur-Eingabe
    kbd = prompt.PROMPT()
