...
uint16_t baseAddr = DEFAULT_ACCESSORY_DECODER_ADDRESS + ADDROFFSET;

CVPair FactoryDefaultCVs[] = {
  { CV_ACCESSORY_DECODER_ADDRESS_LSB, DEFAULT_ACCESSORY_DECODER_ADDRESS + ADDROFFSET },
...
void startup_sequence() {
  // Test Aussenbeleuchtung
  for (int i = 0; i < num_aussenlichter; i++) {
    if (i != TAGESLICHT) {
      for (byte n = 0; n < 3; n++) {
        digitalWrite(AUSSENLICHT_PINS[i], LOW);  // die Ausgänge der Einzel-LED sind LOW-aktiv, ca. 1.5 mA je LED, der Streifen ist HIGH-aktiv, ca. 90 mA
        delay(100);
        digitalWrite(AUSSENLICHT_PINS[i], HIGH);
        delay(100);
      }
    }
  }
  // Test Innenbeleuchtung
  for (byte n = 0; n < 3; n++) {
    for (int raum = 0; raum < LEDS_INNENLICHT; raum++) {
      innenlicht_an(raum);
    }
