...
*/
const char* VERSION[] = { "0", "7b", "20251228.1334" };
...
lv_obj_t* address_switch_loco_reverse;
...
void send_ble(char * command) {
  String value;
  if (command > "") {
    value = convert_cstring_to_string(command);
    pCharacteristic->setValue(value);
    Serial.println("Sent: " + value);
    value = "";
    while (value.substring(0, 3) != ">>>" && value.substring(-3, 3) != "<<<") {
      value = pCharacteristic->getValue();
    }
    if (!strncmp(command, "<<<L?>>>", sizeof("<<<L?>>>"))) {
      parse_values(value.c_str());
    }
#ifdef _DEBUG_
    Serial.println("Finally received: " + value);
#endif
  }
  pCharacteristic->setValue("");
}
...
void parse_values(const char* data) {
...
String convert_cstring_to_string(char * cstr) {
  String str = "";
...
static void address_btn_ok_event_cb(lv_event_t* e) {
  ...
  if (code == LV_EVENT_CLICKED && loco_cnt <= (sizeof(locos) / sizeof(LOCO_TYPE))) {
    ...
    if (lv_obj_has_state(address_switch_loco_reverse, LV_STATE_CHECKED)) {
      active_loco->invers = true;
    }
    else {
      active_loco->invers = true;
    }
...
static void tacho_released_event_cb(lv_event_t* e) {
  lv_event_code_t code = lv_event_get_code(e);
  uint8_t active_speedstep;
  if (code == LV_EVENT_RELEASED) {
    speed = lv_arc_get_value(tacho);
    old_speed = speed;
    active_speedstep = constrain((int)(speed * active_loco->speedsteps / 100), 0, 125);
    ...
    if (forward ^ reversed) {
      ...
      char_cnt = snprintf(temp_cmd, sizeof(temp_cmd), "<<<L%d#%c%d#>>>", (int)active_loco->address, dir, active_speedstep);
    }
  } else if (code == LV_EVENT_VALUE_CHANGED) {
    speed = lv_arc_get_value(tacho);
    if (abs(old_speed - speed) > 10) {
      active_speedstep = constrain((int)(speed * active_loco->speedsteps / 100), 0, 126);
      old_speed = speed;
      if (forward ^ reversed) {
        char dir = active_loco->invers ? (reversed ? 'V' : 'R') : (forward ? 'V' : 'R');
        char_cnt = snprintf(temp_cmd, sizeof(temp_cmd), "<<<L%d#%c%d#>>>", (int)active_loco->address, dir, active_speedstep);
    ...
    lv_label_set_text_fmt(label_tacho, "%d %%\n(%d)", speed, active_speedstep);
  }
}
...
static void loco_list_event_cb(lv_event_t* e) {
  ...
  if (code == LV_EVENT_VALUE_CHANGED) {
    ...
    if(active_loco->invers) {
      lv_obj_set_state(address_switch_loco_reverse, LV_STATE_CHECKED, true);
    }
  }
}
...
void lv_create_tab_address(lv_obj_t* tab) {
  ...
  lv_obj_align(label, LV_ALIGN_CENTER, -48, -32);
  ...
  lv_obj_align(address_spinbox_addr, LV_ALIGN_CENTER, -32, 16);
  ...
  address_switch_loco_reverse = lv_switch_create(tab);
  lv_obj_align(address_switch_loco_reverse, LV_ALIGN_TOP_LEFT, 20, 140);
  label = lv_label_create(tab);
  lv_label_set_text(label, "Fahrtrichtung umkehren");
  lv_obj_align_to(label, address_switch_loco_reverse, LV_ALIGN_OUT_RIGHT_MID, 12, 0);
}
...
void lv_create_tab_info(lv_obj_t* tab) {
  ...
  lv_label_set_text(label, temp_label);
  wireless_label = lv_label_create(tab);
  lv_obj_align(wireless_label, LV_ALIGN_TOP_MID, 0, 72);
  lv_label_set_text(wireless_label, LV_SYMBOL_BLUETOOTH " Characteristics defined");
  layout_label = lv_label_create(tab);
  lv_obj_align(layout_label, LV_ALIGN_TOP_MID, 0, 88);
  snprintf(temp_label, sizeof(temp_label), "%s", BLE_SERVERNAME);
